...
namespace Voat.Data
{
    ...
    {
        public async Task<IEnumerable<int>> GetUserVotes(string userName, SearchOptions options)
        {
            var data = await (from x in _db.Vote
        ...
        //TODO: Creates and Syncs a submission to the vote data
        private async Task PublishVote(Domain.Models.Vote vote, Data.Models.Vote updateDataModel)
        {
            //Create Submission if doesn't exist
            //Sync submission Title / Contents with Vote Record data
            Data.Models.Submission submission = null;
            if (vote.SubmissionID.HasValue)
            {
                submission = GetSubmission(vote.SubmissionID.Value);
                
                //Set data
                submission.Title = vote.Title;
                submission.Content = vote.Content;
                submission.FormattedContent = Formatting.FormatMessage(submission.Content);
                submission.LastEditDate = CurrentDate;
            }
            else
            {
                //This sets all the defaults for submissions
                submission = await CreateSubmission(
                    new UserSubmission(vote.Subverse,
                        new UserSubmissionContent() {
                            Content = vote.ToMarkdown(), Title = vote.Title
                        }){
                    Type = SubmissionType.Vote
                });
                updateDataModel.Status = (int)VoteStatus.Active;
            }
            ...
            //Set Vote Data Model
            updateDataModel.SubmissionID = submission.ID;
        ...
        public async Task<CommandResponse<Domain.Models.Vote>> SaveVote(Domain.Models.Vote vote)
        {
            try
            {
                ...
                else
                {
                    ...
                    newDataModel.Status = (int)VoteStatus.Unpublished;
